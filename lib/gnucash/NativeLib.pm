...
sub qof_log_init() is native(&gncmod-engine) is export { * }

sub qof_init() is native(&gncmod-engine) is export { * }

sub gnc_module_system_init() is native(&gncmod-engine) is export { * }
...
class QofSession is repr('CPointer') is export {
    ...
    # Native methods
    method qof_session_new(-->QofSession) is native(&gncmod-engine) { * }

    method qof_session_begin(Str $book_id, int64 $ignore_lock, int64 $create, int64 $force) is native(&gncmod-engine) { * }

    method qof_session_swap_data(QofSession $session2) is native(&gncmod-engine) { * }

    method qof_session_load(&percentage_func (Str $message, num64 $percent)) is native(&gncmod-engine) { * }
    ...
    method qof_session_ensure_all_data_loaded() is native(&gncmod-engine) { * }

    method qof_session_end() is native(&gncmod-engine) { * }
